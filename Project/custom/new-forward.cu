...
#define IMPL_INPUT_UNROLLING                1
#define IMPL_UNROLLING_KERNEL_FUSION        2
...
#elif (CUR_VERSION == IMPL_UNROLLING_KERNEL_FUSION)

__global__ void conv_forward_kernel_shared(float *output, const float *input, const float *mask, 
    const int B, const int M, const int C, const int H, const int W, const int K,const int S) {
...
#elif (CUR_VERSION == IMPL_INPUT_UNROLLING)
...
__host__ void GPUInterface::conv_forward_gpu_prolog(const float *host_output, const float *host_input, 
...
#elif (CUR_VERSION == IMPL_UNROLLING_KERNEL_FUSION || CUR_VERSION == IMPL_INPUT_UNROLLING)
    ...
    #if(CUR_VERSION == IMPL_INPUT_UNROLLING)
